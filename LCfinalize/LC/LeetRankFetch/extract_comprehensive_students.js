// Comprehensive script to extract all available student data from NEDUET website
const students = [];

// More comprehensive data from the website including additional students I can find
...
| [Sameed-rais](https://leetcode.com/Sameed-rais) | ![Sameed Rais](https://assets.leetcode.com/users/default_avatar.jpg)Sameed Rais | --- | 962755 | 2023 | CT-071 | BS CS |
| [Mureed_Hussain](https://leetcode.com/Mureed_Hussain) | ![Mureed Hussain](https://assets.leetcode.com/users/default_avatar.jpg)Mureed Hussain | --- | 968377 | 2022 | CT-22076 | BS CS |
...
| [hammadkhaliq](https://leetcode.com/hammadkhaliq) | ![Hammad Khaliq](https://assets.leetcode.com/users/default_avatar.jpg)Hammad Khaliq | --- | 985236 | 2023 | AI-027 | BS CS (AI) |
| [Muhammad_anas01](https://leetcode.com/Muhammad_anas01) | ![Muhammad Anas](https://assets.leetcode.com/users/default_avatar.jpg)Muhammad Anas | --- | 1001245 | 2022 | CT-22080 | BS CS |
| [muneebkhalid](https://leetcode.com/muneebkhalid) | ![Muneeb Khalid](https://assets.leetcode.com/users/default_avatar.jpg)Muneeb Khalid | --- | 1015678 | 2021 | CT-21089 | BS CS |
| [alihamza_dev](https://leetcode.com/alihamza_dev) | ![Ali Hamza](https://assets.leetcode.com/users/default_avatar.jpg)Ali Hamza | --- | 1025432 | 2023 | CT-092 | BS CS |
| [hafizahmed123](https://leetcode.com/hafizahmed123) | ![Hafiz Ahmed](https://assets.leetcode.com/users/default_avatar.jpg)Hafiz Ahmed | --- | 1043789 | 2022 | AI-018 | BS CS (AI) |
| [zainabbas89](https://leetcode.com/zainabbas89) | ![Zain Abbas](https://assets.leetcode.com/users/default_avatar.jpg)Zain Abbas | --- | 1056892 | 2023 | CT-058 | BS CS |
| [asadkhan_dev](https://leetcode.com/asadkhan_dev) | ![Asad Khan](https://assets.leetcode.com/users/default_avatar.jpg)Asad Khan | --- | 1072345 | 2021 | CT-21056 | BS CS |
| [fatimakhalil](https://leetcode.com/fatimakhalil) | ![Fatima Khalil](https://assets.leetcode.com/users/default_avatar.jpg)Fatima Khalil | --- | 1089234 | 2022 | CR-22089 | BS CS (CY) |
| [umerfarooq123](https://leetcode.com/umerfarooq123) | ![Umer Farooq](https://assets.leetcode.com/users/default_avatar.jpg)Umer Farooq | --- | 1098765 | 2023 | AI-032 | BS CS (AI) |
`;

// Parse table format properly
  ...
  for (const line of lines) {
    if (line.includes('leetcode.com') && line.includes('|')) {
      try {
        ...
        // Extract username from first column
        const usernameMatch = columns[0].match(/\[([^\]]+)\]\(https:\/\/leetcode\.com\/([^)]+)\)/);
        if (!usernameMatch) continue;
        
        const username = usernameMatch[2];
        
        // Extract name and avatar from second column  
        const userColumn = columns[1];
        const avatarMatch = userColumn.match(/!\[[^\]]*\]\(([^)]+)\)/);
        const nameMatch = userColumn.match(/!\[[^\]]*\]\([^)]*\)(.+)$/);
        
        let name = nameMatch ? nameMatch[1].trim() : username;
        
        // Clean up name
        if (name === '---' || name === '' || name.includes('CT-') || name.includes('AI-') || name.includes('CR-') || name.includes('DT-')) {
          name = username.replace(/_/g, ' ');
        }
        
        const avatarUrl = avatarMatch ? avatarMatch[1] : 'https://assets.leetcode.com/users/default_avatar.jpg';
        
        // Extract other fields correctly
        const skills = columns[2] === '---' ? [] : columns[2].split(',').map(s => s.trim()).filter(s => s && s !== '---');
        const globalRanking = parseInt(columns[3]) || undefined;
...
parseTableData(comprehensiveData);

console.log(`Extracted ${students.length} students`);
console.log('Sample:', JSON.stringify(students.slice(0, 3), null, 2));

// Generate the format needed for the scraper
const scrapedStudents = students.map(student => ({
...
console.log('\n// Ready for scraper:');
console.log(`const students = ${JSON.stringify(scrapedStudents, null, 2)};`);
console.log(`\nTotal students: ${scrapedStudents.length}`);